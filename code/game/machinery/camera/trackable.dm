...
/datum/trackable/process()
	...
	if(cameraticks < CAMERA_TICK_LIMIT)
		if(!cameraticks)
			to_chat(tracking_holder, span_warning("Цель не рядом с активными камерами. Пытаюсь восстановить слежку..."))
		cameraticks++
		return

	to_chat(tracking_holder, span_warning("Не удаётся восстановить слежку, отмена..."))
	reset_tracking()
...
/datum/trackable/proc/track_input(mob/living/tracker)
	...
	var/target_name = tgui_input_list(tracker, "Выберите цель", "Слежка", find_trackable_mobs())
	...
	if(isnull(mob_ref))
		to_chat(tracker, span_notice("Цель не на камерах или рядом нет активных камер. Слежка не удалась."))
		return
...
/datum/trackable/proc/track_name(mob/living/tracker, tracked_mob_name)
	...
	if(isnull(mob_ref))
		to_chat(tracker, span_notice("Цель не на камерах или рядом нет активных камер. Слежка не удалась."))
		return
	to_chat(tracker, span_notice("Начинаю слежку за [tracked_mob_name] по камерам."))
	set_tracked_mob(mob_ref.resolve())
