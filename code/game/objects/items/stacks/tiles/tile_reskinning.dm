...
/obj/item/stack/tile/attack_self(mob/user)
	...
	if(tile_reskin_types && tile_rotate_dirs)
		radial_options["Сменить вид"] = image(icon = 'icons/hud/radial.dmi', icon_state = "radial_reskin")
		radial_options["Повернуть"] = image(icon = 'icons/hud/radial.dmi', icon_state = "radial_rotate")
